...
void incr(int candidates[][9][9], int i, int j, int k, int v) {
    for (int r = 0; r < 9; r++) {
        candidates[i][r][k] += v;
        candidates[r][j][k] += v;
    }
...
int count(int candidates[][9][9], int i, int j, int *v) {
    int c = 0;
    for (int k = 0; k < 9; k++) {
    ...
    */
    ...
    while (1) {
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                int v;
                if (board[i][j] == '.' && count(candidates, i, j, &v) == 1) {
                    // 只有一个候选v
                    incr(candidates, i, j, v, 1);
                    board[i][j] = '0' + v + 1;
                    h = 1;
